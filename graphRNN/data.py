...
class Graph_sequence_sampler_pytorch(torch.utils.data.Dataset):
    def __init__(self, G_list, max_num_node=None, max_prev_node=None, iteration=20000):
        ...
        self.len_all = []
        for G in G_list:
            self.adj_all.append(np.asarray(nx.to_numpy_matrix(G))) # make adj matrix
    ...
    # what the meaning of prev node?
    # why we need to permute the graph
    # why use dfs sequence to re-permute adj matrix
    def calc_max_prev_node(self, iter=20000,topk=10):
        max_prev_node = []
        for i in range(iter):
            if i % (iter / 5) == 0:
                print('iter {} times'.format(i))
            adj_idx = np.random.randint(len(self.adj_all)) # choose one adj matrix(one graph)
            adj_copy = self.adj_all[adj_idx].copy() # copy the matrix
