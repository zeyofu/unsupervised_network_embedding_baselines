...
def load_data(path="data/dblp/", dataset="dblp", diffusion_threshold=10, from_text=True):
    """Load citation network dataset (cora only for now)"""
    print('Loading {} dataset...'.format(dataset))
    ...
    if from_text:
        ...
        with open(path + 'node-feature.csv', 'r') as feature_file:
            num_node = 0
        ...
        diff_graph = {}
        diff_file = open(path + 'author-1900-2020.csv', 'r').readlines()[1:]
        ...
        diff_set = [paper for paper, authors in diff_graph.items() if len(authors)>=diffusion_threshold]

        with open(path + 'paper-feature.csv', 'r') as inf:
            diffusion = {}
            for line in inf:
                paper, feature = line.rstrip().split(',')
                paper = int(paper)
                if paper in diff_set:
                    feature = np.array(list(map(float, feature.split())))
                    diffusion[paper] = (np.array(diff_graph[paper]), feature)

        del diff_file, diff_graph
    else:
